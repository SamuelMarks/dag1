...
type ProxyHandler interface {
	//CommitHandler is called when DAG1 commits a block to the DAG. It
	...
	//SnapshotHandler is called by DAG1 to retrieve a snapshot corresponding to a
	//particular block
	SnapshotHandler(blockIndex int64) (snapshot []byte, err error)

	//RestoreHandler is called by DAG1 to restore the application to a specific
	//state
